...
const createSorter = (videos)=>{
    if (videos.length === 0) {
        return (_options)=>[];
    }
    const sorter = (options)=>{
        let result = videos;
        const isNestVideoArray = (v)=>Array.isArray(result[0]);
        options.forEach((option)=>{
            if (option.sortType === 'groupBy') {
                ...
                const orderer = ({ [option.field]: v1 }, { [option.field]: v2 })=>{
                    let _v1 = v1;
                    let _v2 = v2;
                    ...
                    return option.order === 'ascend' ? _v1 < _v2 ? -1 : 1 : _v1 < _v2 ? 1 : -1;
                };
                if (isNestVideoArray(result)) {
        ...
        console.table(result.flat(1).map((video)=>({
                ...video,
